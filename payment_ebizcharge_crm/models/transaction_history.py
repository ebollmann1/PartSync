...
class TransactionHeader(models.TransientModel):
    ...
    def default_get(self, default_fields):
        ...
        if 'ebiz_profile_id' in rec and 'start_date' in rec and 'end_date' in rec:
            ...
            list_of_transaction = [fields.Command.clear()]
            if list_of_instance_trans:
                ...
                for trans in list_of_instance_trans:
                    list_of_transaction.append(fields.Command.create(trans))
                rec['transaction_lines'] = list_of_transaction
        return rec
    ...
    def get_instance_transaction(self, filters, ebiz_profile=None):
        try:
            list_of_trans = []
            instances = ebiz_profile or self.ebiz_profile_id or self.env['ebizcharge.instance.config'].search(
                [('is_valid_credential', '=', True), ('is_active', '=', True)])
                        ...
                        if transaction['Details']['Invoice'] not in ['Token', 'PM'] and transaction['Details'][
                            ...
                            if transaction['CustomerID'] and transaction['CustomerID'].isnumeric():
                                try:
                                    ...
                                    partner_id = False
                            invoice = sale = False
                            if transaction['Details']['Invoice']:
                                invoice = self.env['account.move'].search([('name', '=', transaction['Details']['Invoice'].split(' ', 1)[0])], limit=1)
                                sale = self.env['sale.order'].search([('name', '=', transaction['Details']['Invoice'].split(' ', 1)[0])], limit=1)
                            if invoice:
                            ...
                            dict1 = {
                                ...
                                'surcharge_percentage': str(instance.surcharge_percentage) + '%' if not transaction['CheckData'] else '0.0%',
    ...
    def action_open_history(self):
        ...
        record = self
        if profile:
            record = self.env['transaction.header'].create({'ebiz_profile_id': profile})
            record.search_transaction()
        return {
...
class TransactionHistory(models.TransientModel):
    ...
    def _process_web_form_payments(self, ebiz, current_pay):
        email_pay_sources = ('email pay', 'Email Pay', 'Email Payment Form', 'email payment form')
        for payment_ebiz in current_pay:
            if self.source not in email_pay_sources:
                continue
            ...
            if self.invoice_id:
                invoice = invoice.search([('name', '=', self.invoice_id.split(' ', 1)[0])], limit=1)
                sale = sale.search([('name', '=', self.invoice_id.split(' ', 1)[0])], limit=1)
            if invoice:
                invoice.write({'save_payment_link': False, 'request_amount': 0, 'last_request_amount': 0})
            if sale:
                sale.write({'save_payment_link': False, 'request_amount': 0, 'last_request_amount': 0})
    ...
    def _get_transaction_details(self):
        return {
            ...
            'Shipping': 0,
            'Discount': 0,
            'Subtotal': self.subtotal or self.amount,
            'AllowPartialAuth': False,
            'Tip': 0,
            'NonTax': True,
            'Duty': 0
    ...
    def execute_transaction(self, kwargs):
        partner = self.env['res.partner'].browse([int(self.customer_id)])
        try:
            instance = partner.ebiz_profile_id
            if instance:
                ebiz = self.env['ebiz.charge.api'].get_ebiz_charge_obj(instance=instance)
    ...
    def action_create_credit_note(self):
        try:
            invoice = self.env['account.move'].search([('name', '=', self.invoice_id)], limit=1)
            if invoice:
                ...
                if provider:
                    invoice_line_vals = [fields.Command.create({
                        'name': self.invoice_id + '-' + "EBizCharge",
                        'quantity': 1,
                    ...
                    move_vals = {
                        ...
                        'payment_reference': self.invoice_id + '-' + "EBizCharge",
    ...
    def credit_or_void(self):
        try:
            ...
            for line in self:
                ...
                instance = (customer and customer.ebiz_profile_id) or line.ebiz_profile_id
                ...
                if line.status in ["Pending", "Settled", "Submitted"]:
                    if 'Check' in line.transaction_type:
                        ...
                        if line.status == 'Pending':
                            ...
                            command = 'Credit'
                        resp = line.execute_transaction({'command': command})
                        if command == 'Void' and resp['ResultCode'] not in ["D", "E"]:
                            trans_ids = self.env['payment.transaction'].search(
                                [('provider_reference', '=', line.ref_no)], limit=1)
                        ...
                        sale = self.env['sale.order']
                        if line.invoice_id:
                            invoice = invoice.search([('name', '=', line.invoice_id.split(' ', 1)[0])], limit=1)
                            sale = sale.search([('name', '=', line.invoice_id.split(' ', 1)[0])], limit=1)
                        resp = ebiz.execute_transaction(line.ref_no, {'command': command},
                                                        transaction_histry_amt=line.subtotal,
                                                        transaction_histry_tax=line.tax, invoice=invoice, sale=sale)

                        if command == 'Void' and resp['ResultCode'] not in ["D", "E"]:
                            trans_ids = self.env['payment.transaction'].search(
                                [('provider_reference', '=', line.ref_no)], limit=1)
                            if trans_ids.state not in ('cancel', 'error') and invoice.payment_state not in (
                            'not_paid', 'reversed', 'invoicing_legacy'):
                                trans_ids.state = 'draft'
                                trans_ids._set_canceled()
                else:
                    continue
                ...
                if resp['ResultCode'] == 'A':
                    if command == 'Credit':
                        line.action_create_credit_note()
                    line._process_web_form_payments(ebiz, current_pay)
            ...
            wizard = self.env['wizard.transaction.history.message'].create({'name': 'Message', 'lines_ids': resp_lines,
                                                                            ...
                                                                            'failed_count': failed, })
            if self:
                self.transaction_id.search_transaction()
            return {
                'type': 'ir.actions.act_window',
                'name': _('Credit/Void'),
    ...
    def _create_email_receipt(self, instance):
        ebiz = self.env['ebiz.charge.api'].get_ebiz_charge_obj(instance=instance)
    ...
    def action_open_email_wizard(self):
        try:
            ...
            # CREATE: email.receipt if it doesn't exist
            self._create_email_receipt(instance)
    ...
    def get_details(self):
        ...
        transactions = ebiz.client.service.GetTransactionDetails(
            **{'securityToken': ebiz._generate_security_json(),
               'transactionRefNum': self.ref_no})

        line_vals = []
        document_type = 'Quick Payment'
        invoice = sale = False
        if self.invoice_id:
            invoice = self.env['account.move'].search([('name', '=', self.invoice_id.split(' ', 1)[0])], limit=1)
            sale = self.env['sale.order'].search([('name', '=', self.invoice_id.split(' ', 1)[0])], limit=1)
        if invoice:
