...
class SyncProducts(models.Model):
    ...
    def compute_company(self):
        ...
        if self.ebiz_profile_id and self.ebiz_profile_id.is_default:
            ...
            if not self.ebiz_profile_id.company_ids:
                existing_companies = profile_obj.search([('is_active', '=', True)]).company_ids.ids
            ...
            companies = profile_obj.search(
                [('is_active', '=', True), '|', ('company_ids', '=', False),
                 ('company_ids', 'in', self.company_id.ids)]).company_ids.ids
        else:
            companies = self.env.context.get('allowed_company_ids')
    ...
    def _compute_button_auto_check(self):
        self.auto_sync_button = self.ebiz_profile_id.ebiz_auto_sync_products if self.ebiz_profile_id else False
    ...
    def _compute_tax_total(self):
        ...
        for tax_id in self.taxes_id:
            if tax_id.amount_type == 'percent':
                tax_amount = (self.list_price * tax_id.amount) / 100
                ...
                for child in tax_id.children_tax_ids:
                    if child.amount_type == 'percent':
                        tax_amount += (self.list_price * child.amount) / 100
                    elif child.amount_type == 'fixed':
                        tax_amount += child.amount
            total_tax += tax_amount
        return total_tax

    def _build_product_details(self, total_tax):
        return {
            'Name': self.name,
            'Description': self.name,
            'UnitPrice': self.list_price,
            'UnitCost': self.standard_price or self.list_price,
            'Active': True,
            'ItemType': 'Service' if self.type == 'service' else 'inventory',
            'SKU': self.barcode or '',
            'Taxable': bool(self.taxes_id),
            'TaxRate': total_tax,
            'ItemNotes': self.description or '',
            'ItemId': self.id,
            'SoftwareId': 'Odoo CRM',
            'QtyOnHand': int(self.qty_available) if hasattr(self, 'qty_available') else 0,
        }

    def _create_product_log(self, status):
    ...
    def _get_or_create_product_ref(self):
        ref = self.env['list.of.products'].search([('product_id', '=', self.id)], limit=1)
        if ref:
            ref.last_sync_date = datetime.now()
        else:
            self.env['list.of.products'].create({
                'product_name': self.id,
                'sync_transaction_id': self.env['upload.products'].search([], limit=1).id,
            })
            ref = self.env['list.of.products'].search([('product_id', '=', self.id)], limit=1)
            if ref:
                ref.last_sync_date = datetime.now()
    ...
    def _sync_single_product(self, ebiz, product_details, from_upload, ref=None):
        resp_line = {'record_name': self.name}
        success = 0
        failed = 0

        if self.ebiz_product_internal_id and self.ebiz_product_id:
            update = ebiz.client.service.UpdateItem(**{
                'securityToken': ebiz._generate_security_json(),
                'itemInternalId': self.ebiz_product_internal_id,
                'itemId': self.ebiz_product_id,
                'itemDetails': product_details,
            })
            resp_line['record_message'] = update['Error'] or update['Status']
            self.sync_status = 'Success'
            self.sync_response = update['Status']
            if self.sync_response == 'Success':
                self.last_sync_date = datetime.now()
                success += 1
            else:
                failed += 1
            self._create_product_log(update['Status'])
            self.sync_status = update['Status']
        else:
            ...
            if create['Status'] == 'Success':
                self.ebiz_product_internal_id = create['ItemInternalId']
                self.ebiz_product_id = self.id
                self.sync_status = 'Success'
                self.sync_response = create['Status']
                self.last_sync_date = datetime.now()
                success += 1
                if from_upload:
                    self._create_product_log(create['Status'])
                    if ref:
                        ref.sync_status = create['Status']
        ...
        return resp_line, success, failed

    def import_ebiz_products(self):
        try:
            instance = self.ebiz_profile_id or False
                ...
                for product in get_all_products:
                    odoo_product = product_obj.search(
                        [('ebiz_product_internal_id', '=', product['ItemInternalId']),
                         ('ebiz_product_id', '=', product['ItemId'])], limit=1)
                    if not odoo_product:
                        product_obj.create({
                            'name': product['Name'],
    ...
    def add_update_to_ebiz(self, from_upload=False):
        try:
            resp_lines = []
            success = failed = 0
            total = len(self)
            for product in self:
                ref = product._get_or_create_product_ref() if from_upload else None
                instance = product.ebiz_profile_id or self.env['ebizcharge.instance.config'].search(
                ...
                product_details = product._build_product_details(product._compute_tax_total())
                resp_line, s, f = product._sync_single_product(
                    ebiz, product_details, from_upload, ref=ref)
                success += s
                failed += f
    ...
    def sync_to_ebiz(self):
        resp = self.add_update_to_ebiz_bulk()
        self._create_product_log(resp['Status'])
        self.write({
    ...
    def add_update_to_ebiz_bulk(self):
        try:
            instance = self.ebiz_profile_id or self.env['ebizcharge.instance.config'].search(
                [('is_valid_credential', '=', True), ('is_default', '=', True)], limit=1)
            if not instance:
                raise ValidationError('Please attach profile on product record or set one of profile to default.')
            if not self.ebiz_profile_id:
                self.ebiz_profile_id = instance.id
            ebiz = self.env['ebiz.charge.api'].get_ebiz_charge_obj(instance=instance)
            product_details = self._build_product_details(self._compute_tax_total())
            if self.ebiz_product_internal_id and self.ebiz_product_id:
                return ebiz.client.service.UpdateItem(**{
                    'securityToken': ebiz._generate_security_json(),
                    'itemInternalId': self.ebiz_product_internal_id,
                    'itemId': self.ebiz_product_id,
                    'itemDetails': product_details,
    ...
    def create(self, val_list):
        products = super().create(val_list)
    ...
    def write(self, values):
        product = super().write(values)
        skip_keys = {'ebiz_product_internal_id', 'ebiz_product_id', 'last_sync_date', 'sync_response', 'sync_status'}
        if not skip_keys.intersection(values):
            for item in self:
                if item.ebiz_product_internal_id:
    ...
    def create(self, values):
        ...
        for rec in record:
            if rec.product_tmpl_id.ebiz_profile_id and rec.product_id.ebiz_product_internal_id and rec.product_id.ebiz_product_id:
                instance = rec.product_id.ebiz_profile_id or None
                ebiz = self.env['ebiz.charge.api'].get_ebiz_charge_obj(instance=instance)
                update = ebiz.client.service.UpdateItem(**{
                    'securityToken': ebiz._generate_security_json(),
                    ...
                    'itemDetails': {
                        'QtyOnHand': rec.quantity if hasattr(rec.product_id, 'qty_available') else 0,
                    },
                })
                self.env['logs.of.products'].create({
    ...
    def write(self, values):
        ...
        for rec in self:
            if rec.product_tmpl_id.ebiz_profile_id:
                ...
                if product.ebiz_product_internal_id and product.ebiz_product_id and 'inventory_quantity' in values:
                    instance = product.ebiz_profile_id or None
                    ebiz = self.env['ebiz.charge.api'].get_ebiz_charge_obj(instance=instance)
                    update = ebiz.client.service.UpdateItem(**{
                        'securityToken': ebiz._generate_security_json(),
                        ...
                        'itemDetails': {
                            'QtyOnHand': rec.quantity if hasattr(product, 'qty_available') else 0,
                        },
                    })
                    self.env['logs.of.products'].create({
